...
export function Orders() {
  ...
  async function changeStatus(id: number, status: DeliveryStatus) {
    await api.patch(`/api/deliveries/${id}/status`, { status, location: 'Atualização pela central' })
    load()
  }

  return (
    <article className="panel">
      <div className="panelHeader"><div><h2>Pedidos e entregas</h2><p>Gestão operacional de ponta a ponta</p></div></div>
      {loading ? <p>Carregando...</p> : (
        <div className="tableWrap">
          <table>
            <thead><tr><th>Pedido</th><th>Cliente</th><th>Código</th><th>Status</th><th>Ação</th></tr></thead>
            <tbody>
              {orders.map(order => (
                <tr key={order.id}>
                  ...
                  <td>
                    <select value={order.status} onChange={e => changeStatus(order.id, e.target.value as DeliveryStatus)}>
