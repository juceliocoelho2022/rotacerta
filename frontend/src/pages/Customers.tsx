...
export function Customers() {
  ...
  useEffect(() => {
    ...
    api.get<CustomerDetail>(`/api/customers/${selectedId}`)
      .then(response => setDetail(response.data))
      .catch(() => setMessage('Não foi possível carregar os detalhes do cliente.'))
  }, [selectedId])
  ...
  const filtered = useMemo(() => {
    ...
    return customers.filter(customer => {
      ...
      const matchesDelivery = deliveryFilter === 'ALL'
        || (deliveryFilter === 'YES' ? customer.activeDeliveries > 0 : customer.activeDeliveries === 0)
      const matchesOccurrence = occurrenceFilter === 'ALL'
        || (occurrenceFilter === 'YES' ? customer.occurrences > 0 : customer.occurrences === 0)
  ...
  const averageRating = customers.length
    ? customers.reduce((sum, customer) => sum + Number(customer.rating), 0) / customers.length
    : 0
  const now = new Date()
  ...
  return (
    <div className="customersPage">
      <section className="customersHeader">
        <div>
          <h1>Clientes</h1>
          <p>Gerencie seus clientes, endereços, preferências e histórico de entregas.</p>
        </div>
      ...
      <section className="customerMetrics">
        {cards.map(([label, value, Icon]) => (
          <article key={label}>
            <div className="customerMetricIcon"><Icon size={20} /></div>
      ...
      <section className="customerToolbar">
        ...
        <select value={status} onChange={event => setStatus(event.target.value)}>
          <option value="ALL">Status</option><option value="ACTIVE">Ativos</option><option value="INACTIVE">Inativos</option>
        </select>
        <select value={city} onChange={event => setCity(event.target.value)}>
          <option value="ALL">Cidade</option>{cities.map(item => <option value={item} key={item}>{item}</option>)}
        </select>
        <select value={deliveryFilter} onChange={event => setDeliveryFilter(event.target.value)}>
          <option value="ALL">Com entrega</option><option value="YES">Sim</option><option value="NO">Não</option>
        </select>
        <select value={occurrenceFilter} onChange={event => setOccurrenceFilter(event.target.value)}>
          <option value="ALL">Ocorrências</option><option value="YES">Com ocorrência</option><option value="NO">Sem ocorrência</option>
        </select>
        <button className="customerPrimaryButton" onClick={() => setMessage('Cadastro de novo cliente será conectado ao formulário na próxima etapa.')}><Plus size={16} /> Novo cliente</button>
      </section>
      ...
      <section className="customerWorkspace">
        ...
        <aside className="customerDrawer">
          {detail ? (
            <>
              <div className="customerDrawerTop">
                <div><h2>{detail.name}</h2><span className={detail.active ? 'customerState active' : 'customerState inactive'}>{detail.active ? '● Ativo' : '● Inativo'}</span></div>
              </div>
              ...
              <div className="customerTabs">
                ...
                ] as const).map(([value, label]) => <button className={tab === value ? 'active' : ''} key={value} onClick={() => setTab(value)}>{label}</button>)}
              </div>
                ...
                <div className="customerTabContent">
                  <section className="customerProfileGrid">
                    ...
                    <div className="customerContacts">
                      <span><Phone size={14} /> {detail.phone ?? 'Telefone não informado'}</span>
                      <span><Mail size={14} /> {detail.email}</span>
                      <span><CalendarDays size={14} /> Cliente desde {formatDate(detail.createdAt)}</span>
                    </div>
                  ...
                  <section className="customerCurrentDelivery">
                    ...
                    {currentOrder ? <>
                      <div className="currentDeliveryBody"><div><strong>Pedido #{currentOrder.orderNumber}</strong><span>{formatMoney(currentOrder.total)}</span></div><StatusBadge status={currentOrder.status} /></div>
                      <button onClick={() => navigate('/tracking')}>Acompanhar entrega</button>
                    </> : <div className="customerEmpty">Nenhuma entrega em andamento.</div>}
                  </section>
                  ...
                  <section className="customerTwoColumns">
                    ...
                    <div className="customerMiniPanel">
                      <div className="sectionTitle"><strong>Pessoas autorizadas</strong></div>
                  ...
                  <section className="customerSummaryStats">
                    <span><small>Pedidos</small><strong>{detail.totalOrders}</strong></span>
                    <span><small>Em andamento</small><strong>{detail.activeDeliveries}</strong></span>
                    <span><small>Ocorrências</small><strong>{detail.occurrences}</strong></span>
                    <span><small>Valor total</small><strong>{formatMoney(detail.totalSpent)}</strong></span>
                  </section>
                </div>
              )}

              {tab === 'addresses' && <div className="customerTabContent"><div className="sectionTitle"><strong>Endereços cadastrados</strong><button><Plus size={14}/> Adicionar</button></div><div className="addressList">{detail.addresses.map(address => <article key={address.id}><MapPin size={17}/><div><strong>{address.label} {address.primaryAddress && <em>Principal</em>}</strong><span>{address.street}, {address.number}{address.complement ? ` • ${address.complement}` : ''}</span><small>{address.district ? `${address.district} • ` : ''}{address.city}/{address.state}</small></div></article>)}</div></div>}
